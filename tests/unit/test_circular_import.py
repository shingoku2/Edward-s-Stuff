...
def main():
    ...
    files = {
        "src/omnix/ai_assistant.py": check_imports("src/omnix/ai_assistant.py"),
        "src/omnix/knowledge_integration.py": check_imports("src/omnix/knowledge_integration.py"),
        "src/omnix/__init__.py": check_imports("src/omnix/__init__.py"),
    }
    ...
    ai_assistant_imports = files["src/omnix/ai_assistant.py"]
    knowledge_imports = files["src/omnix/knowledge_integration.py"]
    init_imports = files["src/omnix/__init__.py"]
    ...
    if has_knowledge_import:
        # Check if it's using omnix. prefix
        src_prefix = any("omnix.knowledge_integration" in imp for imp in ai_assistant_imports)
        print(f"   ✓ YES - Uses 'omnix.' prefix: {src_prefix}")
    else:
        print("   ✗ NO")

    print("\n2. knowledge_integration.py imports from src modules?")
    has_src_imports = any(imp.startswith("omnix.") for imp in knowledge_imports)
    ...
    # The fix is successful if ai_assistant uses omnix. prefix
    ai_uses_src_prefix = any("omnix.knowledge_integration" in imp for imp in ai_assistant_imports)

    if ai_uses_src_prefix:
        print("\n✓ FIXED: All imports use consistent 'omnix.' prefix")
