...
from omnix.capabilities import detect_desktop_capabilities
from omnix.config import Config
from omnix.credential_store import CredentialStore
from omnix.keybind_manager import KeybindManager
from omnix.macro_manager import MacroManager
from omnix.providers_tab import FetchModelsThread
from omnix.settings_dialog import TabbedSettingsDialog
from omnix.ui.design_system import OmnixDesignSystem, design_system
...
class OverlayWindow(QWidget):
    ...
    def __init__(self, assistant, config: Config, ds: OmnixDesignSystem):
        ...
        self.setStyleSheet(
            ds.generate_overlay_stylesheet(float(getattr(config, "overlay_opacity", 0.92)))
        )
        ...
        self.chat_input.setAccessibleName("Overlay question")
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.capabilities = detect_desktop_capabilities()

        self.setWindowTitle("Omnix Gaming Companion")
        self.resize(1280, 800)
        self.setMinimumSize(960, 640)
        stylesheet_factory = getattr(self.design_system, "generate_complete_stylesheet", None)
        self.setStyleSheet(stylesheet_factory() if stylesheet_factory else "")
    ...
    def _build_chat_panel(self) -> QFrame:
        ...
        self.chat_input.setAccessibleName("Ask Omnix")
    ...
    def _build_quick_settings_panel(self) -> QFrame:
        ...
        if not self.capabilities.automation_available:
            automation_notice = QLabel(self.capabilities.reason)
            automation_notice.setObjectName("game-info-label")
            automation_notice.setWordWrap(True)
            automation_notice.setAccessibleName("Desktop automation unavailable")
            layout.addWidget(automation_notice)
    ...
    def _open_settings(self) -> None:
        self._open_settings_tab(0)

    def _open_settings_tab(self, index: int) -> None:
        ...
        self.settings_dialog.set_current_tab(index)
        self.settings_dialog.exec()

    def _open_macro_manager(self) -> None:
        self._open_settings_tab(4)

    def _open_knowledge_manager(self) -> None:
        self._open_settings_tab(2)
    ...
    def closeEvent(self, event) -> None:
        ...
        if self.model_fetch_thread and self.model_fetch_thread.isRunning():
            self.model_fetch_thread.requestInterruption()
