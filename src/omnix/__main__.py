...
def main():
    ...
    try:
        ...
        from omnix.migrations import migrate_user_data

        config_dir = Path(os.getenv("OMNIX_CONFIG_DIR", str(Path.home() / ".gaming_ai_assistant")))
        migration = migrate_user_data(config_dir)
        if migration.changed:
            logger.info(
                "User profile upgraded from schema %s to %s (backup: %s)",
                migration.previous_version,
                migration.current_version,
                migration.backup_dir or "not required",
            )

        config = Config(require_keys=False, config_dir=str(config_dir))
        ...
        _license_key = credential_store.get_credential("omnix", "license_key") or os.getenv(
            "OMNIX_LICENSE_KEY", ""
        )
        ...
        if not _dev_mode and _supabase_url:
            _validator = get_validator(_supabase_url, _supabase_key, credential_store)
            _valid, _msg = _validator.validate(_license_key) if _license_key else (False, "No key")

            if not _valid:
                from omnix.license_dialog import LicenseDialog
