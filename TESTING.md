...
with `python -m pip install --upgrade "setuptools>=83" ".[dev,build]"`; use
`--cov=omnix` for coverage. Ubuntu/Debian hosts also need `libegl1` before
PyQt6 can load in headless mode.
...
```bash
# Install development dependencies and the audited build tool
...
```yaml
Strategy:
- Ubuntu: Python 3.11, with `libegl1`
- Windows: Python 3.11
- macOS: Python 3.11
```

The matrix installs `setuptools>=83` and `.[dev,build]`, validates package
compilation/import, runs the complete suite with XML coverage, and runs Bandit.
All runners set offscreen Qt, a CI-only credential master password, and the
dummy pynput backend.

#### 2. **Code Quality**

- **Flake8** - Linting and syntax checks
- **Bandit** - Security vulnerability scanning
- **pip-audit** - Active-environment dependency vulnerability checks

### Coverage Reporting

Coverage XML is generated by each matrix test job as `coverage.xml`. The
current workflow does not upload it to Codecov or retain it as an artifact.

### Viewing CI/CD Results

- **GitHub Actions:** [View Workflow Runs](https://github.com/shingoku2/Omnix-All-knowing-gaming-companion/actions)
- **Detailed guide:** [CI/CD Pipeline Guide](docs/CI_CD_GUIDE.md)
...
Fixtures are defined in the repository-root `conftest.py` and automatically
available to all tests. It assigns a disposable `OMNIX_CONFIG_DIR` before
application imports, so tests never write to a real user profile.
...
Generate coverage locally with the commands below. The current CI workflow
writes `coverage.xml` but does not publish a Codecov badge or artifact.

### Coverage Goals

| Component | Current | Target | Priority |
|-----------|---------|--------|----------|
| Security (Credentials) | Measure locally | 95%+ | 🔴 Critical |
| AI Providers | Measure locally | 90% | 🔴 High |
| Core Logic | Measure locally | 85% | 🟡 Medium |
| UI Components | Measure locally | 60% | 🟢 Low |
| **Overall** | **Measure locally** | **80%** | - |
...
If Ubuntu reports `ImportError: libEGL.so.1`, install the native runtime first:
...
Offscreen mode does not replace native Qt shared libraries.
...
#### 6. **Path containment differs by operating system**

Resolve both the candidate file and every allowed base before comparing them.
macOS temporary paths can change from `/var/...` to `/private/var/...` after
resolution, and Windows can normalize aliases similarly. Use disposable test
directories and include an alias-path regression case.

#### 7. **pip-audit reports setuptools**

The package build-system requirement is isolated from the active environment.
Upgrade the environment that pip-audit scans:
